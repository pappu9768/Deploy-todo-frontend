...
const ContextProvider = ({ children }) => {
  ...
  const fetchMoviesInPage = async (page = 1, search = "",sortBy = "title", order = "asc") => {
    try {
      ...
      const res = await fetch(`https://deploy-todo-backend.vercel.app/api/v1/auth/movies?page=${page}&limit=10&search=${search}&sortBy=${sortBy}&order=${order}`,{
        headers:{
          'Content-type':'application/json',
          'authorization':`${token}`
        }
  ...
  const getRole = async () => {
    try {
      ...
      const response = await fetch('https://deploy-todo-backend.vercel.app/api/loggedIn', {
        method: 'GET',
        headers: {
          'content-type': 'application/json',
